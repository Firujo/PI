...
public class GaussianRandomVariable {
	...
	public int getY(int quadrant) {
		Integer y;
		int lvlMean = ((height / 2) // altura do ecran de jogo a dividir por 2
									// para ter o valor de altura de um
									// quadrante

		/ 4) // altura de um quadrante a dividir por 4 de modo a ter 3 linhas de
				// orientação para media
		...
		while (true) {
			double rv = lvlDeviation * Math.sqrt(-2 * Math.log((new Random()).nextDouble()))
					* Math.cos(2 * Math.PI * (new Random()).nextDouble()) + lvlMean;
			y = (int) rv;
			if (y > -1 && y < (height / 2)) {
				break;
			}
